...
const loadTestSession = async () => {
  try {
    loading.value = true;
    const response = await fetch(`/api/sessions/token/${props.token}`);
...
const startSession = async () => {
  try {
    await fetch(`/api/sessions/${session.value.id}/start`, {
      method: 'POST'
    });
  } catch (error) {
...
const runCode = async () => {
  ...
  try {
    const response = await fetch('/api/evaluation/run-code', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
const validateSQL = async () => {
  ...
  try {
    const response = await fetch('/api/evaluation/validate-sql', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
